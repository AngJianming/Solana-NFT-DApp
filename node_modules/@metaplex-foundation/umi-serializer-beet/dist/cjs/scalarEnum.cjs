...
function scalarEnum(constructor, options = {}) {
  ...
  function assertValidVariant(variant) {
    ...
    if (isInvalidNumber || isInvalidString) {
      throw new errors.BeetSerializerError(`Invalid enum variant. Got "${variant}", ` + `expected one of [${stringValues.join(', ')}] ` + `or a number between ${minRange} and ${maxRange}`);
    }
  }
  return {
    ...
    deserialize: (bytes, offset = 0) => {
      if (bytes.slice(offset).length === 0) {
        throw new errors.DeserializingEmptyBufferError('enum');
      }
